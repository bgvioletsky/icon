...
function F() {
  ...
  if (fileExist("hiker://files/bgHouse/icon/json/localStorage.json")) {
    ...
    for (let index in list) {
      d.push({
        title: getMyVar("mainindex", "0") == index ?
          "““””<font color='#58D3F7'>" + list[index].dirname + "</font>" : "““””<font color='#22383e'>" + list[index].dirname + "</font>",
      ...
      d.push({
        ...
        extra: {
          longClick: [{
            ...
            title: " 上传 ",
            js: $.toString(( pic, a, b, c) => {
              if (getItem("warehouse") == "1") {
                const method = require((config.依赖).replace("main.js", 'method.js'));
                let dir =pic.split("/").at(-2);
                let f_name=pic.split("/").at(-1);
                method.upload(a,b,c,dir,f_name,pic)
                refreshPage(false);
              toast("图片已上传");
              }else{
                toast("无API无法上传");
            ...
            },  pic_url, getItem("token"), getItem("repoOwner"), getItem("repoName")),
          }],
        },
        pic_url: pic_url,
...
function warehouse() {
  ...
  d.push({
    ...
    pic_url: getItem("warehouse", "0") == "0" ?
      "https://cdn.jsdelivr.net/gh/bgvioletsky/test/system/kai.svg" : "https://cdn.jsdelivr.net/gh/bgvioletsky/test/system/guan.svg",
    col_type: "text_icon",
  }, {
    title: "网络链接",
    url: $("#noLoading#").lazyRule(
...
function Imgsaving() {
  if (getItem("warehouse", "0") == "1") {
    ...
    for (let index in list) {
      let imgpath = [];
     let ss= method.getImg(
        "/storage/emulated/0/Android/data/com.example.hikerview/files/Documents/bgHouse/icon/src/" +
        list[index].dirpath
      )
      imgpath=imgpath.concat(ss);
      
      for(let i in imgpath){
        imgpath[i]="https://cdn.jsdelivr.net/gh/" +
        getItem("repoOwner") +
        "/" +
        getItem("repoName") +
        "/" +
      ...
      writeFile("hiker://files/bgHouse/icon/json/"+list[index].dirpath+".json",JSON.stringify(imgpath));
    }
  }
}